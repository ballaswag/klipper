...
send_error(struct hx71x_adc *hx71x, uint8_t oid, uint8_t error_code) {
    // clear pending flag, stop scheduling reads
...
hx71x_read_adc(struct hx71x_adc *hx71x, uint8_t oid)
{
    ...
    // is_ready flag should have be cleared after reading. If this is true we
    // dont understand the sensor state and need to reset
    if (is_ready) {
        send_error(hx71x, oid, ERROR_READY_AFTER_READ);
        return;
    }

    // A read should take no longer than 1/2 the sample interval
    // If this is true the MCU is probably overloaded
